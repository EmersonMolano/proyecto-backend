...
public class UsuarioTareaService {
    ...
    public boolean asignarTarea(UUID usuarioId, UUID tareaId) {
        if (!usuarioRepository.existsById(usuarioId) || !tareaRepository.existsById(tareaId)) {
            return false; // usuario o tarea no existen
        }
    ...
    public List<UsuarioTarea> findAll() {
        return usuarioTareaRepository.findAll();
    }
